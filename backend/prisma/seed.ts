...
async function main() {
  ...
  // ═══════════════════════════════════════════════
  const assetData: { code: string; name: string; type: string; address: string; city: string; lat: number; lng: number; w: number; h: number; lighting: string; rate: number; traffic: number; landmark: string; vendorIdx: number; status?: string }[] = [
    // ── RAIPUR (30 assets) ──
  ...
  for (let i = 0; i < assetData.length; i++) {
    ...
    const asset = await prisma.asset.create({
      data: {
        ...
        status: (a.status || statuses[i % statuses.length]) as any,
        address: a.address, city: a.city, state: 'Chhattisgarh',
        latitude: a.lat, longitude: a.lng, width: a.w, height: a.h,
        lighting: a.lighting as any, faces: a.type === 'GANTRY' ? 2 : 1,
  ...
  for (const c of campaignDefs) {
    const campaign = await prisma.campaign.create({
      data: {
        ...
        status: c.status as any, startDate: new Date(c.start), endDate: new Date(c.end),
  ...
  // ═══ BOOKINGS (40) ═══
  // ═══════════════════════════════════════════════
  const bookingDefs: { campIdx: number; assetIdx: number; start: string; end: string; status: string; amount: number }[] = [
  ...
  for (const b of bookingDefs) {
    const booking = await prisma.booking.create({
      data: {
        ...
        status: b.status as any, amount: b.amount,
  ...
  for (const inv of invoiceDefs) {
    ...
    await prisma.invoice.create({
      data: {
        ...
        status: inv.status as any, dueDate: new Date(inv.due),
  ...
  // ═══ NOTIFICATIONS (40) ═══
  // ═══════════════════════════════════════════════
  const notifDefs: { userId: string; type: string; title: string; message: string; isRead: boolean; daysAgo: number }[] = [
    // Admin notifications
  ...
  for (const n of notifDefs) {
    ...
    await prisma.notification.create({
      data: { userId: n.userId, type: n.type as any, title: n.title, message: n.message, isRead: n.isRead, createdAt },
    });
  }
  ...
  // ═══ ACTIVITY LOGS (50) ═══
  // ═══════════════════════════════════════════════
  const activityDefs: { userId: string; action: string; entity: string; entityId?: string; desc: string; daysAgo: number }[] = [
  ...
  for (const eq of enquiryDefs) {
    ...
    if (eq.assignedToId) data.assignedToId = eq.assignedToId;
    if ((eq as any).clientId) data.clientId = (eq as any).clientId;
    if ((eq as any).campaignId) data.campaignId = (eq as any).campaignId;
    if ((eq as any).lostReason) data.lostReason = 'Budget constraints - postponed to next quarter';
    if (eq.status === 'CONVERTED') data.convertedAt = new Date(Date.now() - 20 * 24 * 60 * 60 * 1000);
